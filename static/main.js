document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get DOM elements for import/export functionality
    ...
    if (importFileInput) {
        importFileInput.addEventListener('change', function(e) {
            debugLog('File selected for import');
    ...
    function exportGameData() {
        try {
            ...
            const exportData = {
                ...
                games: {},
                playerState: Utils.loadPlayerState()
            };
            
            // Get all game-related localStorage data
            const keys = Object.keys(localStorage);
            
            // Export chat histories
            const gameHistoryKeys = keys.filter(key => key.startsWith('chatHistory_game_'));
            gameHistoryKeys.forEach(key => {
                const gameId = key.replace('chatHistory_game_', '');
                const historyData = localStorage.getItem(key);
                if (historyData) {
                    try {
                        exportData.games[gameId] = {
                            chatHistory: JSON.parse(historyData),
                            timestamp: Date.now()
                        };
                    } catch (e) {
                        console.warn(`Failed to parse history for game ${gameId}:`, e);
                    }
                }
            });
            ...
            gameDataKeys.forEach(key => {
                const gameId = key.replace('gameData_game_', '');
                const gameData = localStorage.getItem(key);
                if (gameData && exportData.games[gameId]) {
                    try {
                        exportData.games[gameId].gameData = JSON.parse(gameData);
                    } catch (e) {
                        console.warn(`Failed to parse game data for game ${gameId}:`, e);
                    }
                }
            });
            ...
            link.download = `ai-dungeon-backup-${new Date().toISOString().split('T')[0]}.json`;
            ...
            addSystemMessage(`✓ Data exported successfully! ${Object.keys(exportData.games).length} game(s) exported.`, false, false, true);
            debugLog(`Export completed. Games exported: ${Object.keys(exportData.games).length}`);
            
        } catch (error) {
            console.error("Error exporting data:", error);
            addSystemMessage("❌ Error exporting data. Check console for details.", false, false, true);
        }
    }
    ...
    function importGameData(file) {
        if (!file) {
            addSystemMessage("❌ No file selected for import.", false, false, true);
            return;
        }
        
        if (!file.name.endsWith('.json')) {
            addSystemMessage("❌ Please select a valid JSON file.", false, false, true);
            return;
        }
        ...
        reader.onload = function(e) {
            try {
                ...
                if (!importData.version || !importData.games) {
                    addSystemMessage("❌ Invalid backup file format.", false, false, true);
                    return;
                }
                ...
                Object.entries(importData.games).forEach(([gameId, gameData]) => {
                    try {
                        ...
                        const existingHistory = localStorage.getItem(`chatHistory_game_${gameId}`);
                        const existingGameData = localStorage.getItem(`gameData_game_${gameId}`);
                        
                        if (existingHistory || existingGameData) {
                            // Game exists, ask user what to do
                            const overwrite = confirm(`Game ${gameId} already exists. Overwrite?`);
                            if (!overwrite) {
                                skippedGames++;
                                return;
                            }
                        ...
                        if (gameData.chatHistory) {
                            localStorage.setItem(`chatHistory_game_${gameId}`, JSON.stringify(gameData.chatHistory));
                        }
                        
                        // Import game data
                        if (gameData.gameData) {
                            localStorage.setItem(`gameData_game_${gameId}`, JSON.stringify(gameData.gameData));
                        }
                        
                        importedGames++;
                        
                    } catch (gameError) {
                        console.warn(`Failed to import game ${gameId}:`, gameError);
                        skippedGames++;
                    }
                });
                
                // Restore player state if available
                if (importData.playerState) {
                    localStorage.setItem('playerState', JSON.stringify(importData.playerState));
                    if (importData.playerState.names) {
                        playerNames = importData.playerState.names;
                        updatePlayerLabels();
                        debugLog("Restored player names:", playerNames);
                    }
                }
                
                // Restore model selections
                if (importData.selectedModel) {
                    selectedModel = importData.selectedModel;
                ...
                if (importData.selectedImageModel) {
                    selectedImageModel = importData.selectedImageModel;
                    localStorage.setItem('selectedImageModel', selectedImageModel);
                    if (currentImageModelName) {
                        currentImageModelName.textContent = selectedImageModel;
                    }
                }
                
                // Show import results
                let resultMessage = `✓ Import completed! ${importedGames} game(s) imported.`;
                if (skippedGames > 0) {
                    resultMessage += ` ${skippedGames} game(s) skipped.`;
                }
                
                addSystemMessage(resultMessage, false, false, true);
                debugLog(`Import completed. Games imported: ${importedGames}, skipped: ${skippedGames}`);
                
                // Offer to reload to see imported games
                if (importedGames > 0) {
                    setTimeout(() => {
                        const reload = confirm("Import successful! Reload the page to see imported games?");
                        if (reload) {
                            location.reload();
                        }
                    }, 2000);
                ...
                addSystemMessage("❌ Error importing data. Invalid file format.", false, false, true);
            }
        };
        
        reader.onerror = function() {
            addSystemMessage("❌ Error reading file.", false, false, true);
        };
